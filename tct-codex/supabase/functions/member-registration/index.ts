...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const rollbackRegistration = async () => {
      await admin.from('member_email_verifications').delete().eq('user_id', data.user.id)
    ...
    if (verificationError) {
      ...
      return Response.json({ error: 'Bestätigungscode konnte nicht erstellt werden.' }, { status: 500, headers: corsHeaders })
    }
